...
-   It's following an SIR model (as opposed to SEIR, i.e. no latency, or no delay between infection and infectiousness)
...
ggplot(data = hcm23 %>%
  ...
  geom_line(aes(x = as.character(date),
                ...
                group =1,col = "infectious contact rate"),alpha = 0.3,inherit.aes = FALSE,
            lwd = 2) +
  scale_color_manual(values = c("infectious contact rate" = "blue"),
                     name="Analysis Type")+
  ...
  scale_y_continuous(
    ...
    sec.axis = sec_axis(~./15000000, name="Infectious contact rate")
  )+
  theme_minimal()+
  theme(axis.text.x = element_text(size = 18),
        axis.text.y = element_text(size = 18),
...
hfmd |> 
  ...
  facet_wrap(~factor(collection,levels = c(6:9),
...
unc_gam <- hfmd |> 
  ...
  scale_y_continuous(labels = scales::label_percent(scale = 100),limits = c(0,1))+
  geom_segment(aes(x = 6, y = 0.5, xend = 5.02, yend = 0.218),
               arrow = arrow())+
  annotate("text", x= 6.1, y=0.55, label= "A",size = 5)+
  geom_segment(aes(x = 4.5, y = 0.6, xend = 5.02, yend = 0.371),
               arrow = arrow())+
  annotate("text", x= 4.6, y=0.65, label= "B",size = 5)+
  geom_segment(aes(x = 6, y = 0.05, xend = 5.02, yend = 0.116),
               arrow = arrow())+
  annotate("text", x= 6.1, y=0.04, label= "C",size = 5)+
  theme_bw()+ 
  theme(axis.title.x = element_text(size = 18),
        axis.text.x = element_text(size = 18),
        legend.position = "none",
...
ber <- hfmd |> 
  ...
  group_modify(~ .x |>
                 age_profile(age_values, ci)) |> 
  filter(collection == 6 & age > 4.99999 & age < 5.03) |> 
  ...
  ggplot(aes(x = factor(seropositvty))) +  # treat 0/1 as categories
  geom_bar(fill = "steelblue")+
  facet_wrap(~factor(line,labels = c("Fit (A)","Upper (B)","Lower (C)")),)+
  labs(y = "Count",x = "Seropositivity",title = "Bernoulli distribution")+
...
constr %>%
  ...
  geom_point()+
  facet_wrap(vars(factor(line,
                         levels = c("upr","fit","lwr")),
...
ts2 <- ggplot()+
  geom_bar(data = df_plot, aes(x = as.Date(adm_week), y = n),stat = "identity",
           alpha = 0.5) +
  geom_point(data = atdf,aes(x = col_date, y = pos*3000),shape = "|")+
  labs(x = "Admission week",y = "Cases")+
...
- Publication: What we can and cannot observe from serosurveillance compared to case notification data.

- Serological Analysis: Estimate attack rates using serological samples; compare these with hospitalisation incidence to calculate the probability of hospitalisation upon infection as a function of age.

- Modeling: Identify the timeline of summer breaks from 2013 to 2023 to apply the temporal force of infection model and explore transmission dynamics.
